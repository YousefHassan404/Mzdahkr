...
export default function RentManagerPage() {
  ...
  React.useEffect(() => {
    const fetchRents = async () => {
      try {
        const res = await axios.get('http://localhost:5000/api/rent');
        setRents(res.data);
      } catch (error) {
    ...
    <div className="flex ml-64 min-h-screen bg-gray-100 text-right rtl">
      ...
      <div className="flex-1 p-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-800">إدارة الوحدات للإيجار</h1>
          <Link
            to="/admin-dashboard/rent/new"
          ...
          {rents.map((rent) => (
            <div key={rent._id} className="bg-white rounded shadow p-4 space-y-2">
              ...
              <div className="flex gap-2 pt-2">
                <Link
                  to={`/admin-dashboard/rent/edit/${rent._id}`}
                  className="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded"
                >
                  تعديل
                </Link>
                <Link
                  to={`/admin-dashboard/rent/delete/${rent._id}`}

...
//     url: `http://localhost:5000/api/units/${id}`,
//     key: ["share", id],
//   });
...
//   const maxImages = 3;
//   const showImages = unit.images?.length > maxImages ? unit.images.slice(0, maxImages) : unit.images;

//   const maxVideos = 2;
//   const showVideos = !showAllVideos && unit.videos?.length > maxVideos ? unit.videos.slice(0, maxVideos) : unit.videos;
...
//             <div>
//               <h1 className="text-3xl font-bold text-gray-900 mb-2">{unit.type}</h1>
//               <p className="text-gray-500">{unit.location?.address}, {unit.location?.city}</p>
//             </div>
...
//           <h2 className="text-xl font-semibold text-gray-800 mb-4">معرض الصور</h2>
//           <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
//             {showImages?.map((img, idx) => (
//               <div
//                 key={idx}
//                 className="relative aspect-square overflow-hidden rounded-lg cursor-pointer hover:shadow-md transition-all"
//                 onClick={() => setOpen(true)}
//               >
//                 <img src={img} alt={`Property ${idx + 1}`} className="w-full h-full object-cover" />
//                 {idx === maxImages - 1 && unit.images?.length > maxImages && (
//                   <div className="absolute inset-0 bg-black bg-opacity-50 flex justify-center items-center text-white text-lg font-bold">
//                     +{unit.images.length - maxImages} صور إضافية
//                   </div>
//                 )}
//               </div>
//             ))}
...
//         <Lightbox open={open} close={() => setOpen(false)} slides={unit.images?.map((img) => ({ src: img })) || []} plugins={[Thumbnails]} />

//         <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
//           <div className="lg:col-span-2 bg-white rounded-xl shadow-sm p-6 border border-gray-100">
//             <h2 className="text-xl font-semibold text-gray-800 mb-6 pb-2 border-b border-gray-200">تفاصيل الوحدة</h2>
...
//                 <DetailItem label="المقدم " value={`${unit.installmentPlan.downPayment?.toLocaleString()} جنيه`} />
//                 <DetailItem label="مدة التسديد " value={`${unit.installmentPlan.duration?.toLocaleString()} شهر`} />
//                 <DetailItem label="قيمة القسط " value={`${unit.installmentPlan.monthlyInstallment?.toLocaleString()} جنيه`} />
...
//                 <p style={{ direction: "rtl" }}>الموقع علي Google Maps</p>
//                 <a href={`${unit.locationUrl}`} target="_blank" rel="noopener noreferrer">عرض الموقع</a>
//               </div>
//               <div className="pt-4 mt-4 border-t border-gray-200">
//                 <h3 className="text-lg font-medium text-gray-800 mb-2">السعر المعروض</h3>
//                 <p className="text-2xl font-bold text-green-600">{unit.offeredPrice?.toLocaleString()} جنيه</p>
//               </div>
//               <a
...
//             <h2 className="text-xl font-semibold text-gray-800 mb-6 pb-2 border-b border-gray-200">معلومات الاستثمار</h2>
//             <div className="space-y-4">
//               <InfoCard title="ضمانات" value="عقد موثق" description="ضمانات قانونية كاملة" />
//             </div>
//           </div>
//         </div>
...
//             <h2 className="text-xl font-semibold text-gray-800 mb-6 pb-2 border-b border-gray-200">المساهمون الحاليون</h2>
...
//                     <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">نسبة الملكية</th>
//                   </tr>
//                 </thead>
//                 <tbody className="bg-white divide-y divide-gray-200">
//                   {unit.users.map((user, idx) => (
//                     <tr key={idx}>
//                       <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">المستخدم #{user.userId}</td>
...
//             <h2 className="text-xl font-semibold text-gray-800 mb-4">الفيديوهات</h2>
//             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
...
//               ))}
//             </div>
//             {!showAllVideos && unit.videos.length > maxVideos && (
//               <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-xl shadow-md p-6">
              ...
              <div className="space-y-5">
                ...
                <div className="pt-4 mt-4 border-t border-gray-200">
                  <h3 className="text-lg font-medium text-navy-800 mb-2">السعر المعروض</h3>
                  <p className="text-2xl font-bold text-orange-600">{unit.offeredPrice?.toLocaleString()} جنيه</p>
                </div>
                <a
                  href={`https://wa.me/${unit.ownerPhone || "201234567890"}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center mt-6 px-6 py-3 bg-orange-600 hover:bg-orange-700 text-white font-medium rounded-lg transition-colors shadow-md hover:shadow-lg"
                >
            ...
            {unit.videos?.length > 0 && (
              <section className="bg-white rounded-xl shadow-md p-6">
                <h2 className="text-xl font-semibold text-navy-800 mb-4">الفيديوهات</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {showVideos.map((video, idx) => (
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              <h2 className="text-xl font-semibold text-navy-800 mb-6 pb-2 border-b border-gray-200">معلومات الاستثمار</h2>
              <div className="space-y-4">
                <InfoCard 
                  title="ضمانات" 
                  value="عقد موثق" 
                  description="ضمانات قانونية كاملة وفقًا للقوانين المحلية" 
                />
                <InfoCard 
                  title="العائد المتوقع" 
                  value="8-12% سنويًا" 
                  description="بناءً على تحليل السوق المحلي" 
                />
                <InfoCard 
                  title="مدة الاستثمار" 
                  value="3-5 سنوات" 
                  description="متوسط مدة الاستثمار الموصى بها" 
                />
              </div>
            </div>
